...
var Skype = new function () {
		...
		this.chatLinks = new this.SkypeUriNameLinks("chat with us", "");
		this.multiChatLinks = new this.SkypeUriNameLinks("chat with us", "");
			...
			if (typeof this.analyzeSkypeUri === "function") {
				...
				if (r === "init") {
					...
					if (r === "chat with us") {
						...
						t.prop13 = document.domain + " - chat with us"
					} else {
						if (r === "call") {
		...
		function l(L) {
			...
			if (this.name === this.dropdownLinks.name) {
				...
				if (this.name === this.chatLinks.name) {
					v = "Skype.tryAnalyzeSkypeUri('chat with us', '" + this.analyzeCrumbIndex + "');" + v
				} else {
